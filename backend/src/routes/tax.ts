...
import { calculatePakistanTax, isDeductible, getFbrSection, getMissedDeductionHints } from '../services/taxCalculator';
...
router.get('/summary', async (req: Request, res: Response) => {
  ...
  const incomeTx = await db.prepare(
    `SELECT SUM(amount) as total FROM transactions WHERE user_id = ? AND type = 'income' AND TO_CHAR(date, 'YYYY') = ?`
  ).get(userId, String(CURRENT_TAX_YEAR)) as any;

  const allExpenseTx = await db.prepare(
    `SELECT category, SUM(amount) as total FROM transactions WHERE user_id = ? AND type = 'expense' AND TO_CHAR(date, 'YYYY') = ? GROUP BY category ORDER BY total DESC`
  ...
  const taxProfile = await db.prepare('SELECT * FROM tax_profiles WHERE user_id = ? AND tax_year = ? ORDER BY created_at DESC LIMIT 1').get(userId, CURRENT_TAX_YEAR) as any;
  ...
  const allTx = await db.prepare(`SELECT * FROM transactions WHERE user_id = ? AND type = 'expense' AND TO_CHAR(date, 'YYYY') = ?`).all(userId, String(CURRENT_TAX_YEAR));
...
router.get('/profile', async (req: Request, res: Response) => {
  const userId = req.user!.userId;
  const db = getDb();
  const profile = await db.prepare('SELECT * FROM tax_profiles WHERE user_id = ? AND tax_year = ? ORDER BY created_at DESC LIMIT 1').get(userId, CURRENT_TAX_YEAR) as any;
  if (profile) {
    res.json({
      id: profile.id,
...
router.post('/profile', async (req: Request, res: Response) => {
  ...
  const existing = await db.prepare('SELECT id FROM tax_profiles WHERE user_id = ? AND tax_year = ?').get(userId, CURRENT_TAX_YEAR) as any;

  if (existing) {
    await db.prepare(`UPDATE tax_profiles SET country = ?, cnic = ?, dependents = ?, additional_income = ?, extra_deductions = ?, exemption_claims = ?, filing_status = ?, updated_at = NOW() WHERE id = ?`)
      .run(
        country || 'PK',
        cnic || null,
    ...
    const id = uuidv4();
    await db.prepare(`INSERT INTO tax_profiles (id, user_id, tax_year, country, cnic, dependents, additional_income, extra_deductions, exemption_claims, filing_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(
...
router.post('/chat', async (req: Request, res: Response) => {
  ...
  const allTx = await db.prepare(`SELECT * FROM transactions WHERE user_id = ? AND type = 'expense' AND TO_CHAR(date, 'YYYY') = ?`).all(userId, String(CURRENT_TAX_YEAR));
  const missedDeductions = getMissedDeductionHints(allTx, deductibleExpenses);
