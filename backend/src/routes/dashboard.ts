...
router.get('/', (req: Request, res: Response) => {
  try {
    ...
    const activeGoal = db.prepare('SELECT name, current_amount, target_amount, target_date FROM goals WHERE user_id = ? ORDER BY created_at DESC LIMIT 1').get(userId) as any;
